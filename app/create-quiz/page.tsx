...
export default function NewExam() {
 const [numOfQues, setNumOfQues] = useState(5)
  const [config,setConfig] = useState({ title: '', durationMinutes: 10, startTime: '', endTime: '' ,totalMarks: 100,marksPerQues: 20 })
        ...
        <div className="max-w-4xl mx-auto bg-gray-700 p-6 rounded shadow">
          ...
          <form onSubmit={handleCreate} className="space-y-1">
            {/* custom config */}
                ...
                <label className="text-sm font-medium text-gray-300 w-40">Quiz Title</label>
                <input value={config.title} onChange={e => setConfig({...config, title: e.target.value})} className="flex-1 border p-2 rounded" />
                </div>
                <div className='flex flex-wrap gap-4'>
                <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-gray-300 w-40">Duration (Minutes)</label>
                <input value={config.durationMinutes} type='number' onChange={e => setConfig({...config, durationMinutes: parseInt(e.target.value)})} className="flex-1 border p-2 rounded" />
                </div>
                <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-gray-300 w-40">Marks Per Question </label>
                <input value={config.marksPerQues} type='number' onChange={e => setConfig({...config, marksPerQues: parseInt(e.target.value)})} className="flex-1 border p-2 rounded" />
                </div>
               
                <div className="flex items-center gap-2">
                <label className="text-sm font-medium text-gray-300 w-40">Start Time</label>
                <input value={config.startTime} type='date' onChange={e => setConfig({...config, startTime: e.target.value})} className="flex-1 border p-2 rounded" />
                </div>
                <div className="flex items-center gap-2 ml-16">
                <label className="text-sm font-medium text-gray-300 w-40">End Time</label>
                <input value={config.endTime} type='date' onChange={e => setConfig({...config, endTime: e.target.value})} className="flex-1 border p-2  rounded" />
                </div>
                </div>
              ...
              {questions.map((q, qi) => (
                ...
                <p className='text-white'>{ q.serial}.</p>
                  <input value={q.text} onChange={e => { const copy = [...questions]; copy[qi].text = e.target.value; setQuestions(copy) }} className="w-full m-2 border p-2 rounded" />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {q.choices.map((c: any, ci: number) => (
                      <div key={ci} className="flex gap-2 items-center">
                       
                        <input type="radio" name={`correct-${qi}`} checked={!!c.isCorrect} onChange={() => { const copy = [...questions]; copy[qi].choices.forEach((cc: any) => cc.isCorrect = false); copy[qi].choices[ci].isCorrect = true; setQuestions(copy) }} />
                        <input value={c.text} onChange={e => { const copy = [...questions]; copy[qi].choices[ci].text = e.target.value; setQuestions(copy) }} className="flex-1 border p-1 rounded" />
                      </div>
                    ))}
                  </div>
                </div>
